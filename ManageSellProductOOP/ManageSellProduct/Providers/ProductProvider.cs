...
namespace ManageSellProduct.Providers
{
    public class ProductProvider
    {
        ...
        public static ProductModel[] GetProducts()
        {
            ...
            if (data.Length > 0)
            {
                products = new ProductModel[data.Length];
        ...
        public static ProductModel GetProductByCode(string productCode)
        {
        ...
        private static ProductModel StringToProduct(string data)
        {
            ProductModel product = new ProductModel();    
        ...
        private static string ProductToString(ProductModel product, string formatDate = CommonEnum.Date)
        {
        ...
        {
            CategoryModel[] categories = CategoryProvider.GetCategorys();
            if (categories.Length > 0)
            {
                for (int i = 0; i < categories.Length; i++)
                {
                    if (code == categories[i].CategoryCode) {
                        code = categories[i].CategoryName;
                        break;
                    }
                }
            }
